...
function Booking() {
  ...
  function updateSize(event) {
    ...
    if (value.length === 2) {
      setShoes((prevState) => prevState.map((shoe) => (shoe.id === name ? { ...shoe, size: value } : shoe)));
    }
  }
  ...
  async function book() {
    if (booking.when && booking.lanes && booking.time && shoes.length > 0 && comparePeopleAndShoes()) {
      const bookingInfo = {
  ...
  return (
    <section className="booking">
      ...
      <Shoes updateSize={updateSize} addShoe={addShoe} removeShoe={removeShoe} shoes={shoes} />
